"""Shared synthetic live-provider benchmark; deterministic numeric expectations and saved replay."""
...
async def run_live_acceptance(language: LanguageRuntime, artifact: Path) -> None:
    ...
    assert config.maximum_input_tokens <= 8000 and config.maximum_output_tokens <= (
        2048 if language.provider == "gemini" else 512
    )
    ...
    workspace = CsvDemoWorkspace(CsvDemoSettings(enabled=True, maximum_language_questions=20), language)
    try:
        ...
        async with workspace.lease(token) as item:
            ...
            for name, question, dimension, one_day in CASES:
                ...
                assert trace and trace.provider == language.provider and trace.model, name
                ...
                original_language = workspace.language
                workspace.language = build_language_runtime()
                try:
                    replay = await workspace.rerun(item, str(result.receipt.query_id))
                finally:
                    workspace.language = original_language
