...
class ClaudeConfiguration(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    enabled: bool = False
    ...
    def activation_contract(self) -> Self:
        if self.enabled and (
            ...
            raise ValueError(
                "Claude activation requires an explicit model, approved egress, tenants and metrics."
            )
    ...
    @classmethod
    def load(cls, path: Path) -> ClaudeConfiguration:
        if not path.is_absolute():
            raise ValueError("Claude configuration requires an absolute private file path.")
